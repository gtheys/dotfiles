...
function Find-WindotsRepository {
    ...
    if (-not $profileSymbolicLink) {
        Write-Verbose "No symbolic link found, using profile path directly"
        return Split-Path $ProfilePath
    }

    if (-not $profileSymbolicLink.Target) {
        Write-Verbose "Target not found, using profile path directly"
        return Split-Path $ProfilePath
    }

    Write-Verbose "Using symbolic link target"
